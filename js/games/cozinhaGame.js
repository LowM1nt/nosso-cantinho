  ...
  function update() {
    ...
    items = items.filter(it => {
      if (it.y >= bowl.y - 2 && it.y <= bowl.y + bowl.h && it.x >= bowl.x - 4 && it.x <= bowl.x + bowl.w + 4) {
        if (it.bad) { meter = Math.max(0, meter - 1); flash('#D72638'); msgEl.textContent = 'Eca! esse não 😝'; if (onWrong) onWrong(); }
        else { meter++; flash('#34d399'); msgEl.textContent = 'Hmm, delícia 😋'; }
